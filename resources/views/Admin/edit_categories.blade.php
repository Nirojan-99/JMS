...
<div class="container mt-3 fs-base fw-bold">
    ...
    <div class="m-auto my-1 p-md-5 p-xs-0 container w-75 w-xs-100">
        <div class="rounded shadow-lg p-md-4 p-2">
            <form method="POST" action="{{ route('update_categories',['id' => $category->category_id]) }}" enctype="multipart/form-data" novalidate class="needs-validation">
                @csrf
                @method('put')
                <div class="row align-items-stretch">
                    <div class="col-md-3">
                        <div class="fs-6 mb-2">Upload Image</div>
                        <div class="d-flex align-items-center justify-content-center img_container p-0" id="img_label">
                            <div class="position-relative w-100 h-100">
                                <label for="imageInput" class="w-100 h-100 " id="label">
                                    <img id="imagePreview" src="{{ asset('img/images/category/' . $category->image) }}" alt="Upload Image" class="selected_img position-absolute ">
                                    <img src="{{ URL('img/icon/plus-solid.svg') }}" id="plusIcon" alt="Upload Image" class="add_img_container position-absolute d-none">
                                </label>
                                <img id="trashIcon" src="{{ URL('img/icon/trash-can-solid.svg') }}" class="trash_img_container position-absolute ">
                            </div>
                        </div>
                    ...
                    <div class="col-md-9 d-flex flex-column align-items-end">
                        ...
                        <input value="{{$category->name}}" type="text" class="rounded w-100 px-3 py-1" id="categoryName" placeholder="Category name" name="name" class="form-control" id="validationCustomUsername" aria-describedby="inputGroupPrepend" required>
                        <div id="errMsg" class="pt-1 err_mg text-left w-100 fs-base fw-bold"></div>
